...
// Digital services use exactly the same verified payment providers, but their
// state becomes PAID / IN_PROGRESS instead of entering product dispatch.
router.get('/service-orders/:orderId/options', authMiddleware, async (req, res) => {
  const c = authedClient(req); const { data: order } = await c.from('digital_service_orders').select('client_id,status').eq('id', req.params.orderId).maybeSingle();
  if (!order || order.client_id !== req.user.id) return res.status(404).json({ error: 'Project not found.' });
  if (order.status !== 'AWAITING_PAYMENT') return res.status(409).json({ error: 'This project is not ready for payment.' });
  const { data, error } = await supabase.from('payment_methods').select('method_code,display_name,method_type,provider_code,currency,network,public_details').eq('is_enabled', true).contains('supported_purposes', ['digital_service']).order('priority');
  if (error) return res.status(400).json({ error: error.message }); res.json(data || []);
});

router.post('/service-orders/:orderId/initialize/:provider', authMiddleware, async (req, res) => {
  try {
    const provider = String(req.params.provider || '').toUpperCase(); if (!['PAYSTACK', 'MONNIFY'].includes(provider)) return res.status(400).json({ error: 'This online payment provider is not supported.' });
    if (await emergencyActive('pause_payment_initialization')) return res.status(503).json({ error: 'Payment initialization is temporarily paused by platform operations.' });
    if (!await providerEnabled(provider)) return res.status(503).json({ error: `${provider} is not enabled by the superadmin.` });
    const c = authedClient(req); const { data: order } = await c.from('digital_service_orders').select('*').eq('id', req.params.orderId).maybeSingle();
    if (!order || order.client_id !== req.user.id || order.status !== 'AWAITING_PAYMENT') return res.status(409).json({ error: 'This project is not ready for payment.' });
    let { data: transaction } = await c.from('payment_transactions').select('*').eq('service_order_id', order.id).maybeSingle();
    if (!transaction) {
      const { data: payment, error: paymentError } = await c.from('payments').insert({ service_order_id: order.id, client_id: req.user.id, worker_id: order.provider_id, amount: order.price, service_fee: 0, payment_method: provider.toLowerCase(), purpose: 'escrow', status: 'pending' }).select().single();
      if (paymentError) throw paymentError;
      const { data, error } = await c.from('payment_transactions').insert({ payment_id: payment.id, service_order_id: order.id, buyer_id: req.user.id, seller_id: order.provider_id, skillbridge_reference: `SB-DG-P-${Date.now().toString().slice(-8)}${Math.random().toString(36).slice(2, 5).toUpperCase()}`, provider_code: provider, amount: order.price, currency: order.currency, status: 'PAYMENT_CREATED', verification_status: 'UNVERIFIED' }).select().single();
      if (error) throw error; transaction = data;
    }
    const callbackUrl = `${req.protocol}://${req.get('host')}/service-order.html?id=${order.id}`;
    const checkout = provider === 'PAYSTACK'
      ? await paystack.initialize({ email: req.user.email, amount: transaction.amount, reference: transaction.skillbridge_reference, callbackUrl, metadata: { skillbridge_service_order_id: order.id, skillbridge_reference: transaction.skillbridge_reference } })
      : await monnify.initialize({ amount: transaction.amount, reference: transaction.skillbridge_reference, email: req.user.email, name: req.user.display_name, redirectUrl: callbackUrl, metadata: { skillbridge_service_order_id: order.id } });
    const checkoutUrl = provider === 'PAYSTACK' ? checkout.authorization_url : checkout.checkoutUrl;
    await c.from('payment_transactions').update({ provider_code: provider, provider_reference: transaction.skillbridge_reference, status: 'PAYMENT_INITIALIZED', metadata: { checkout_url: checkoutUrl } }).eq('id', transaction.id);
    await c.from('digital_service_orders').update({ status: 'PAYMENT_PROCESSING', updated_at: new Date().toISOString() }).eq('id', order.id);
    res.json({ checkout_url: checkoutUrl, reference: transaction.skillbridge_reference });
  } catch (error) { res.status(400).json({ error: error.message }); }
});
...
router.post('/webhooks/paystack', async (req, res) => {
  ...
  try {
    ...
    if (transaction.service_order_id) {
      const { data: project } = await supabase.from('digital_service_orders').select('client_id,provider_id,title_snapshot').eq('id', transaction.service_order_id).maybeSingle();
      await supabase.from('digital_service_orders').update({ status: 'PAID', paid_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', transaction.service_order_id);
      await supabase.from('digital_service_events').insert({ service_order_id: transaction.service_order_id, event_code: 'PAYMENT_VERIFIED', previous_status: 'PAYMENT_PROCESSING', next_status: 'PAID', details: { provider: 'PAYSTACK', provider_transaction_id: verified.id } });
      if (project) await notify(supabase, { userId: project.provider_id, type: 'digital_service_paid', title: 'Digital service payment verified', body: `${project.title_snapshot} is paid. You can begin work.`, link: `/service-order.html?id=${transaction.service_order_id}` });
    } else {
