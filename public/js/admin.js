document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadLogistics() {
    const [providers, orders, templates, paymentProviders, paymentMethods, digitalSettings] = await Promise.all([API.get('/logistics/providers'), API.get('/orders/admin/all'), API.get('/orders/admin/templates'), API.get('/admin/payment-providers'), API.get('/admin/payment-methods'), API.get('/digital-services/settings')]);
    document.getElementById('adminMain').innerHTML = `<h1 class="section-title">Orders & Logistics</h1><p class="section-sub">Provider credentials are server-side configuration only. Do not enter secret keys in browser-visible fields.</p>
      ...
      <h3 style="margin-top:28px">Transaction center</h3><p class="card-meta">Payment confirmation can only come from a configured provider webhook followed by server-side verification. Reconcile repeats provider verification; it never accepts a payment claim.</p>${orders.map(o=>{const i=o.product_order_items?.[0]||{},state=o.order_state||o.status;return `<div class="card" style="margin:8px 0"><div class="card-body" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div><strong>${i.title_snapshot||'Order'}</strong><div class="card-meta">${o.order_code} · ${state} · ${fmtPrice(o.total_amount)}</div></div><div style="display:flex;gap:6px"><a class="btn btn-outline btn-sm" href="/order.html?id=${o.id}" target="_blank">View</a>${['PAYMENT_PROCESSING','PAYMENT_PENDING','AWAITING_PAYMENT'].includes(state)?`<button class="btn btn-outline btn-sm" onclick="reconcileOrderPayment('${o.id}')">Reconcile payment</button>`:''}${state==='REFUND_REQUESTED'?`<button class="btn btn-gold btn-sm" onclick="reviewProductRefund('${o.id}',true)">Approve refund</button><button class="btn btn-outline btn-sm" onclick="reviewProductRefund('${o.id}',false)">Reject</button>`:''}</div></div></div>`}).join('')||'<p>No product transactions yet.</p>'}
