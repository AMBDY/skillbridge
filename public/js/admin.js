document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function load(sec) {
    ...
    try {
      ...
      if (sec === 'god-eye') return await loadGodEye();
      if (sec === 'emergency') return await loadEmergencyControls();
  ...
  async function loadGodEye() {
    ...
    document.getElementById('adminMain').innerHTML = `<h1 class="section-title">Eagle Eye Control Room</h1><p class="section-sub">Cross-platform operations view. Financial verification remains server-side and cannot be changed from this screen.</p>
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>Eagle Eye transaction search</h3><div style="display:flex;gap:8px;flex-wrap:wrap"><input class="form-input" id="godEyeSearch" placeholder="Order, payment reference, agreement, tracking number" style="flex:1"><select class="form-select" id="godEyeStatus"><option value="">All statuses</option><option>AWAITING_PAYMENT</option><option>PAYMENT_PROCESSING</option><option>READY_FOR_DISPATCH</option><option>IN_TRANSIT</option><option>BUYER_CONFIRMATION_PENDING</option><option>REFUND_REQUESTED</option><option>COMPLETED</option></select><button class="btn btn-gold" onclick="searchGodEye()">Search</button></div><div id="godEyeResults" style="margin-top:14px">${renderGodEyeResults(monitor)}</div><div id="godEyeDetail" style="margin-top:14px"></div></div></div>
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>Top Sellers Management</h3><p class="card-meta">AI recommends qualifying completed product sellers. Your manual choices always control the public order.</p><label><input type="checkbox" id="tsAi" ${sellers.settings?.ai_recommendations_enabled ? 'checked' : ''}> AI recommendations</label> &nbsp; <label><input type="checkbox" id="tsAuto" ${sellers.settings?.automatic_ranking_enabled ? 'checked' : ''}> Automatic ranking</label> &nbsp; <label><input type="checkbox" id="tsManual" ${sellers.settings?.manual_curation_enabled ? 'checked' : ''}> Manual curation</label> <button class="btn btn-outline btn-sm" onclick="saveTopSellerSettings()">Save controls</button>
      <h3 style="margin-top:20px">⭐ Featured Top Sellers</h3><div id="featuredTopSellers">${featured.length ? featured.map((entry,index) => `<div class="card" style="margin:7px 0;padding:10px;display:flex;justify-content:space-between;align-items:center"><span>${index ? '☰' : '🥇'} <strong>${entry.profile?.display_name || 'Account'}</strong> · ${entry.profile?.completed_sales || 0} completed sales · ⭐${entry.profile?.rating || 0}</span><span><button class="btn btn-outline btn-sm" onclick="moveTopSeller(${index},-1)">↑</button><button class="btn btn-outline btn-sm" onclick="moveTopSeller(${index},1)">↓</button><button class="btn btn-outline btn-sm" onclick="removeTopSeller('${entry.user_id}')">Remove</button></span></div>`).join('') : '<p>No manual selections yet.</p>'}</div>
      <h3 style="margin-top:20px">🤖 AI Recommended Top Sellers</h3>${recommendations.length ? recommendations.map(person => `<div class="card" style="margin:7px 0;padding:10px;display:flex;justify-content:space-between"><span><strong>${person.display_name}</strong> (${person.role}) · Score ${person.ai_score}/100 · ${person.completed_sales} completed sales · ⭐${person.rating || 0}</span><button class="btn btn-gold btn-sm" onclick="promoteTopSeller('${person.user_id}')">Promote</button></div>`).join('') : '<p>No qualifying recommendations yet.</p>'}
      <details style="margin-top:18px"><summary>All accounts with product sales</summary>${accounts.filter(person => person.completed_sales > 0).map(person => `<div style="padding:8px 0;border-bottom:1px solid var(--border)"><strong>${person.display_name}</strong> · ${person.role} · ${person.completed_sales} sales · ⭐${person.rating || 0} <button class="btn btn-outline btn-sm" onclick="promoteTopSeller('${person.user_id}')">Add</button></div>`).join('') || '<p>No completed product sales yet.</p>'}</details></div></div>`;
    window.__featuredTopSellers = featured;
  }
  async function loadEmergencyControls() {
    const rows = await API.get('/admin/emergency-controls');
    document.getElementById('adminMain').innerHTML = `<h1 class="section-title">Emergency Controls</h1><p class="section-sub">These controls require a reason and create an audit record. Use only during an operational incident.</p><div class="card"><div class="card-body">${rows.map(row => `<div style="padding:14px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><div><strong>${row.control_key.replaceAll('_',' ')}</strong><div class="card-meta">${row.is_active ? `ACTIVE · ${row.reason || ''}` : 'Normal operation'}</div></div><button class="btn ${row.is_active ? 'btn-outline' : 'btn-gold'} btn-sm" onclick="setEmergencyControl('${row.control_key}',${!row.is_active})">${row.is_active ? 'Release control' : 'Activate control'}</button></div>`).join('')}</div></div>`;
  }
  window.setEmergencyControl = async (key, is_active) => { const reason = prompt(`${is_active ? 'Why activate' : 'Why release'} ${key.replaceAll('_',' ')}?`); if (!reason) return; if (!confirm(`Confirm: ${is_active ? 'activate' : 'release'} this emergency control?`)) return; try { await API.put(`/admin/emergency-controls/${key}`, { is_active, reason }); Toast.show('Emergency control updated'); load('emergency'); } catch(e) { Toast.show(e.message); } };
  function renderGodEyeResults(data) { const rows = [...(data.orders || []).map(row => `<div style="padding:7px 0;border-bottom:1px solid var(--border)">Order <button class="btn btn-outline btn-sm" onclick="viewGodEyeOrder('${row.id}')">${row.order_code}</button> · ${row.order_state||row.status} · ${fmtPrice(row.total_amount)}</div>`), ...(data.transactions || []).map(row => `<div style="padding:7px 0;border-bottom:1px solid var(--border)">Payment ${row.skillbridge_reference} · ${row.status}</div>`), ...(data.agreements || []).map(row => `<div style="padding:7px 0;border-bottom:1px solid var(--border)">Agreement ${row.agreement_number} · ${row.status}</div>`), ...(data.shipments || []).map(row => `<div style="padding:7px 0;border-bottom:1px solid var(--border)">Shipment ${row.tracking_number || row.shipment_code} · ${row.status}</div>`)]; return rows.length ? rows.join('') : '<p>No matching operational records.</p>'; }
  window.searchGodEye = async () => { try { const data = await API.get(`/admin/god-eye?q=${encodeURIComponent(document.getElementById('godEyeSearch').value)}&status=${encodeURIComponent(document.getElementById('godEyeStatus').value)}`); document.getElementById('godEyeResults').innerHTML = renderGodEyeResults(data); } catch(e) { Toast.show(e.message); } };
  window.viewGodEyeOrder = async id => { try { const data = await API.get(`/admin/god-eye/orders/${id}`); const o=data.order,i=o.product_order_items?.[0]||{}; document.getElementById('godEyeDetail').innerHTML=`<div class="card"><div class="card-body"><h3>Transaction drill-down: ${o.order_code}</h3><p>${i.title_snapshot||'Product'} · ${o.order_state||o.status} · ${fmtPrice(o.total_amount)}</p><p><strong>Payment:</strong> ${data.payment?.skillbridge_reference||'Not initialized'} · ${data.payment?.status||''} · ${data.payment?.provider_code||''}</p><details><summary>Agreement versions (${data.agreement_versions.length})</summary>${data.agreement_versions.map(v=>`<p>v${v.version_number} · ${v.status} · ${new Date(v.created_at).toLocaleString()}</p>`).join('')}</details><details><summary>Timeline (${data.events.length})</summary>${data.events.map(e=>`<p>${new Date(e.created_at).toLocaleString()} — ${e.event_code}: ${e.previous_state||''} → ${e.next_state||''}</p>`).join('')}</details><details><summary>Audit trail (${data.audit.length})</summary>${data.audit.map(a=>`<p>${new Date(a.created_at).toLocaleString()} — ${a.action}</p>`).join('')}</details><a class="btn btn-outline btn-sm" href="/order.html?id=${o.id}" target="_blank">Open user view</a></div></div>`; } catch(e) { Toast.show(e.message); } };
  window.saveTopSellerSettings = async () => { try { await API.put('/admin/top-sellers/settings',{ai_recommendations_enabled:document.getElementById('tsAi').checked,automatic_ranking_enabled:document.getElementById('tsAuto').checked,manual_curation_enabled:document.getElementById('tsManual').checked});Toast.show('Top seller controls saved'); } catch(e) { Toast.show(e.message); } };
  ...
  async function loadUsers() {
    ...
    document.getElementById('adminMain').innerHTML = `
      ...
      <div class="card" style="margin:18px 0;padding:16px"><h3>Completion and hiring-rate rules</h3><p class="card-meta">Actual jobs and KYC-approved reviews calculate rates normally. Use a rule only when you need an administrator-controlled display rate for a user, role, specialty, or new accounts.</p><div class="grid grid-4"><select class="form-select" id="metricName"><option value="completion">Completion</option><option value="hiring">Hiring</option></select><select class="form-select" id="metricScope"><option value="user">Specific user ID</option><option value="role">Role</option><option value="specialty">Specialty category ID</option><option value="new_user">New users (days)</option></select><input class="form-input" id="metricScopeValue" placeholder="User ID, role, category ID, or days"><input class="form-input" id="metricRate" type="number" min="0" max="100" placeholder="Rate %"></div><button class="btn btn-gold btn-sm" style="margin-top:10px" onclick="addProfileMetricRule()">Add rate rule</button>${rules.length ? `<div style="margin-top:12px">${rules.map(r => `<p>${r.metric_name} · ${r.scope_type}: ${r.scope_value || 'all'} → <strong>${r.rate}%</strong> <button class="btn btn-outline btn-sm" onclick="deleteProfileMetricRule('${r.id}')">Delete</button></p>`).join('')}</div>` : ''}</div>
  ...
  window.addProfileMetricRule = async () => { try { await API.post('/admin/profile-metric-rules', { metric_name: document.getElementById('metricName').value, scope_type: document.getElementById('metricScope').value, scope_value: document.getElementById('metricScopeValue').value.trim(), rate: Number(document.getElementById('metricRate').value) }); Toast.show('Rate rule added'); load('users'); } catch(e) { Toast.show(e.message); } };
  window.deleteProfileMetricRule = async id => { if (!confirm('Delete this rate rule?')) return; try { await API.del(`/admin/profile-metric-rules/${id}`); Toast.show('Rate rule deleted'); load('users'); } catch(e) { Toast.show(e.message); } };
  ...
  async function loadKyc() {
    ...
    document.getElementById('adminMain').innerHTML = `
      ...
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>AI-assisted duplicate account monitoring</h3><p class="card-meta">Checks repeated KYC names and identical uploaded document/selfie files. Flags require your review; accounts are never rejected automatically.</p><button class="btn btn-outline btn-sm" onclick="scanKycDuplicates()">Run duplicate scan</button><div style="margin-top:10px">${duplicateChecks.map(x=>`<p>Flag: ${x.match_type.replaceAll('_',' ')} · confidence ${x.confidence}% <button class="btn btn-outline btn-sm" onclick="reviewKycDuplicate('${x.id}','resolved')">Resolve</button><button class="btn btn-outline btn-sm" onclick="reviewKycDuplicate('${x.id}','dismissed')">Dismiss</button></p>`).join('') || 'No open duplicate flags.'}</div></div></div>
  ...
  async function loadLogistics() {
    ...
    document.getElementById('adminMain').innerHTML = `<h1 class="section-title">Orders & Logistics</h1><p class="section-sub">Provider credentials are server-side configuration only. Do not enter secret keys in browser-visible fields.</p>
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>Payment providers</h3><p class="card-meta">Secrets remain server environment variables. Enable only a provider whose secret and webhook URL are configured.</p>${paymentProviders.map(p=>`<div style="padding:9px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><span><strong>${p.display_name}</strong> · ${p.provider_code} · ${p.is_enabled?'Enabled':'Disabled'}<br><small>Secret: ${p.secret_env_key}; webhook secret: ${p.webhook_secret_env_key||'provider-specific'}</small></span><span><button class="btn btn-outline btn-sm" onclick="togglePaymentProvider('${p.id}',${!p.is_enabled})">${p.is_enabled?'Disable':'Enable'}</button><button class="btn btn-outline btn-sm" onclick="makeDefaultPaymentProvider('${p.id}')">Make default</button></span></div>`).join('')}</div></div>
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>Public Fiat & Crypto Payment Methods</h3><p class="card-meta">Add or edit the account details users see. Changing a wallet or bank details updates the live payment-method pages. Do not place API keys here.</p><button class="btn btn-gold btn-sm" onclick="addPaymentMethod()">+ Add payment method</button>${paymentMethods.map(m=>`<div style="padding:10px 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><span><strong>${m.display_name}</strong> · ${m.method_type} · ${m.currency}${m.network?' · '+m.network:''}<br><small>${m.is_enabled?'Visible to users':'Hidden'} · ${m.method_code}</small></span><span><button class="btn btn-outline btn-sm" onclick="editPaymentMethod('${m.id}')">Edit</button><button class="btn btn-outline btn-sm" onclick="togglePaymentMethod('${m.id}',${!m.is_enabled})">${m.is_enabled?'Hide':'Show'}</button><button class="btn btn-outline btn-sm" onclick="deletePaymentMethod('${m.id}')">Delete</button></span></div>`).join('') || '<p>No payment methods added.</p>'}</div></div>
      <div class="card" style="margin:16px 0"><div class="card-body"><h3>Digital service rules</h3><p class="card-meta">These controls apply to project briefs, provider approval, revisions and the digital-delivery workflow.</p><div class="grid grid-2"><label><input type="checkbox" id="dsProviderApproval" ${digitalSettings.require_provider_approval?'checked':''}> Require provider approval</label><label><input type="checkbox" id="dsClientAcceptance" ${digitalSettings.require_client_acceptance?'checked':''}> Require client acceptance</label><label><input type="checkbox" id="dsRevisions" ${digitalSettings.allow_revisions?'checked':''}> Allow revisions</label><label><input type="checkbox" id="dsMilestones" ${digitalSettings.allow_milestones?'checked':''}> Allow milestones</label><label><input type="checkbox" id="dsAutoRelease" ${digitalSettings.auto_release?'checked':''}> Automatic payment release</label><div class="form-group"><label class="form-label">Default included revisions</label><input class="form-input" id="dsDefaultRevisions" type="number" min="0" value="${digitalSettings.default_revisions ?? 2}"></div><div class="form-group"><label class="form-label">Maximum file size (MB)</label><input class="form-input" id="dsMaxFileSize" type="number" min="1" value="${digitalSettings.max_file_size_mb ?? 25}"></div></div><button class="btn btn-gold btn-sm" style="margin-top:12px" onclick="saveDigitalServiceSettings()">Save digital service rules</button></div></div>
      ...
      <h3 style="margin-top:28px">Transaction center</h3><p class="card-meta">Reconcile performs provider verification. Manual override is an exceptional, audited recovery action and requires a reason.</p>${orders.map(o=>{const i=o.product_order_items?.[0]||{},state=o.order_state||o.status;return `<div class="card" style="margin:8px 0"><div class="card-body" style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap"><div><strong>${i.title_snapshot||'Order'}</strong><div class="card-meta">${o.order_code} · ${state} · ${fmtPrice(o.total_amount)}</div></div><div style="display:flex;gap:6px"><a class="btn btn-outline btn-sm" href="/order.html?id=${o.id}" target="_blank">View</a>${['PAYMENT_PROCESSING','PAYMENT_PENDING','AWAITING_PAYMENT','PAYMENT_FAILED','PAYMENT_EXPIRED'].includes(state)?`<button class="btn btn-outline btn-sm" onclick="reconcileOrderPayment('${o.id}')">Reconcile payment</button><button class="btn btn-gold btn-sm" onclick="overrideOrderPayment('${o.id}')">Manual override</button>`:''}${state==='REFUND_REQUESTED'?`<button class="btn btn-gold btn-sm" onclick="reviewProductRefund('${o.id}',true)">Approve refund</button><button class="btn btn-outline btn-sm" onclick="reviewProductRefund('${o.id}',false)">Reject</button>`:''}</div></div></div>`}).join('')||'<p>No product transactions yet.</p>'}
      ...
      <h3 style="margin-top:28px">Digital service projects</h3>${digitalOrders.map(o=>`<div class="card" style="margin:8px 0"><div class="card-body" style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap"><span><strong>${o.title_snapshot}</strong><br><small>${o.order_reference} · ${o.status} · ${fmtPrice(o.price)}</small></span><span><a class="btn btn-outline btn-sm" target="_blank" href="/service-order.html?id=${o.id}">View / override</a></span></div></div>`).join('')||'<p>No digital service projects yet.</p>'}`;
    window.__providers = providers;
    window.__paymentMethods = paymentMethods;
  }
  ...
  window.togglePaymentProvider = async (id, is_enabled) => { try { await API.put(`/admin/payment-providers/${id}`,{is_enabled}); Toast.show(`Provider ${is_enabled?'enabled':'disabled'}`);load('logistics') } catch(e) { Toast.show(e.message); } };
  window.makeDefaultPaymentProvider = async id => { try { await API.put(`/admin/payment-providers/${id}`,{is_default:true}); Toast.show('Default provider updated');load('logistics') } catch(e) { Toast.show(e.message); } };
  window.addPaymentMethod = async () => { const display_name=prompt('Method name (for example, USDT TRC20 or Zenith Bank)'); if(!display_name)return; const method_type=prompt('Type: online_provider, fiat_bank, or crypto_wallet','crypto_wallet'); if(!['online_provider','fiat_bank','crypto_wallet'].includes(method_type))return Toast.show('Invalid payment method type'); const method_code=prompt('Unique method code',display_name.toUpperCase().replace(/[^A-Z0-9]+/g,'_')); if(!method_code)return; const currency=prompt('Currency','NGN')||'NGN'; const network=method_type==='crypto_wallet'?(prompt('Network (for example TRC20)')||null):null; const detail=prompt(method_type==='crypto_wallet'?'Wallet address':'Public details (JSON, e.g. {"bank_name":"...","account_number":"..."})',method_type==='crypto_wallet'?'{"wallet_address":""}':'{}'); try{await API.post('/admin/payment-methods',{display_name,method_type,method_code,currency,network,public_details:JSON.parse(detail||'{}'),is_enabled:false});Toast.show('Payment method added as hidden');load('logistics')}catch(e){Toast.show(e instanceof SyntaxError?'Details must be valid JSON.':e.message)} };
  window.editPaymentMethod = async id => { const m=(window.__paymentMethods||[]).find(x=>x.id===id);if(!m)return; const display_name=prompt('Method name',m.display_name);if(display_name===null)return; const currency=prompt('Currency',m.currency);if(currency===null)return; const network=prompt('Network (blank for none)',m.network||'');if(network===null)return; const detail=prompt('Public details JSON',JSON.stringify(m.public_details||{}));if(detail===null)return;try{await API.put(`/admin/payment-methods/${id}`,{display_name,currency,network:network||null,public_details:JSON.parse(detail)});Toast.show('Payment method updated');load('logistics')}catch(e){Toast.show(e instanceof SyntaxError?'Details must be valid JSON.':e.message)}};
  window.togglePaymentMethod = async (id,is_enabled) => {try{await API.put(`/admin/payment-methods/${id}`,{is_enabled});load('logistics')}catch(e){Toast.show(e.message)}};
  window.deletePaymentMethod = async id => {if(!confirm('Delete this payment method?'))return;try{await API.del(`/admin/payment-methods/${id}`);load('logistics')}catch(e){Toast.show(e.message)}};
  window.saveDigitalServiceSettings = async () => { try { await API.put('/digital-services/settings',{require_provider_approval:document.getElementById('dsProviderApproval').checked,require_client_acceptance:document.getElementById('dsClientAcceptance').checked,allow_revisions:document.getElementById('dsRevisions').checked,allow_milestones:document.getElementById('dsMilestones').checked,auto_release:document.getElementById('dsAutoRelease').checked,default_revisions:Number(document.getElementById('dsDefaultRevisions').value||0),max_file_size_mb:Number(document.getElementById('dsMaxFileSize').value||25)});Toast.show('Digital service rules saved'); } catch(e) { Toast.show(e.message); } };
  ...
  async function loadMessages() {
    ...
    document.getElementById('adminMain').innerHTML = `
      ...
      <div class="card" style="margin:20px 0"><div class="card-body">
        ...
        <div class="form-group"><label class="form-label">Message reason</label><select class="form-select" id="broadcastReason"><option value="welcome">Welcome message</option><option value="update">Platform update</option><option value="latest_news">Latest news</option><option value="announcement">Announcement</option><option value="broadcast">Broadcast</option><option value="maintenance">Maintenance / downtime</option><option value="other">Other — specify below</option></select><input class="form-input" id="broadcastCustomReason" placeholder="Reason if Other" style="margin-top:8px"></div>
        <div class="form-group"><label class="form-label">Delivery mode</label><select class="form-select" id="broadcastMode"><option value="manual">Manual — send now</option><option value="automatic">Automatic campaign record</option></select></div>
        ...
        <div class="form-group"><label class="form-label">Optional image or video</label><input class="form-input" type="file" id="broadcastMedia" accept="image/*,video/*"></div>
  ...
  window.sendBroadcast = async () => {
    ...
    const reason_code = document.getElementById('broadcastReason').value, delivery_mode = document.getElementById('broadcastMode').value, custom_reason = document.getElementById('broadcastCustomReason').value.trim();
    ...
    try {
      const file=document.getElementById('broadcastMedia').files[0], media_url=file?await Upload.file(file):null;
      const { sent } = await API.post('/admin/broadcast', { title, body, target_role, reason_code, delivery_mode, custom_reason, media_url });
  ...
  async function loadHomepageBuilder() {
    ...
    const metricDefaults = { verified_users: '12K+', total_transactions: '₦2.4B', satisfaction_rate: '98%', support_availability: '24/7' };
    const metrics = settings.homepage_metrics || {};
    document.getElementById('adminMain').innerHTML = `
